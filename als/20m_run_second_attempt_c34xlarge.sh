# Convert ratings from movielens format to VW format (54s c3.4xlarge time)
...
date; for i in `seq 0 3`; do  # 6m41s
  for j in `seq 0 3`; do
    generate_matrix $i $j &
  done
done
...
date; for i in `seq 0 3`; do # 59m51s
  for j in `seq 0 3`; do
...
date; for i in `seq 0 3`; do # 1h36m36s
  for j in `seq 0 3`; do
    predict_matrix $i $j &
  done
done
...
date; for i in `seq 0 3`; do # 48m42s
  for j in `seq 0 3`; do
